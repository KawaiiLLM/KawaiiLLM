...
|Token|作用|所在侧|
|---|---|---|
|`<mem>` / `</mem>`|标记 memory token 序列的开始/结束边界|**仅 LLM**|
|`<mempad>`|占位符（reserved）|—|
|`<AE>`|提示 LLM 进入"文本重建"模式（参考 PCC）|LLM|

> **MemE 侧不使用 `<mem>` / `</mem>`**：Qwen3-Embedding-4B 采用 causal attention + last-token pooling。MEM tokens 直接拼接在 context 末尾，最后一个 MEM token (Q_N) 占据 last position，自然继承预训练的全局语义聚合能力。若在 Q_N 之后插入 `</mem>`，则 last position 被 `</mem>` 占据但其 hidden state 被丢弃，浪费了最有价值的聚合位置。
...
在输入文本末尾拼接 N 个可学习的 MEM token embeddings（Query Embeddings）。利用 causal attention 的性质，每个 MEM token 能 attend 到前面的所有 text tokens，从而提取压缩信息。Q_N 占据 last position，继承 Qwen3-Embedding 的 last-token-pool 全局聚合能力。

```
MemE 输入: [text tokens (≤4096)] [Q_1, Q_2, ..., Q_N]
                                   ↑ 可学习的 Query Embeddings，无 <mem>/</mem> 包裹
MemE 输出: 取最后 N 个位置的 last hidden states → [v_1, v_2, ..., v_N]
```

其中 N 在训练时按 batch 采样（见上文"Batch-level n_mem"），最大值 128，即 mem_embeddings 定义为最大长度，按需截取前 n 个：

```python
self.mem_embeddings = nn.Embedding(128, hidden_size)

# encode_context 中动态截取：
mem_embeds = self.mem_embeddings.weight[:n_mem]  # 截取前 n_mem 个
```
...
接收 Projector 输出的向量，以 `[<mem>, h_1, ..., h_N, </mem>]` 作为 prefix 输入 LLM，结合训练目标生成文本。
...
##### 任务分配 — 前驱感知 2-task 确定性轮换

任务分配根据样本是否有**前驱 chunk** 区分：

**有前驱 chunk 的样本** — 2-task 轮转（重建 / 续写）：
```python
TASK_TYPES_HAS_PREV = ["reconstruction", "continuation"]
task_idx = (sample_idx + epoch) % 2
```
- 有前驱可作为 MemE context，两种任务均涉及 MemE + Projector

**无前驱 chunk 的样本** — 2-task 轮转（NTP / 重建）：
```python
TASK_TYPES_NO_PREV = ["ntp", "reconstruction"]
task_idx = (sample_idx + epoch) % 2
```
- 文档首 chunk 或独立短 chunk，参与 NTP + 重建，不参与续写

续写方向：**前驱 chunk → MemE context，当前 chunk → LLM target**。与推理场景一致——将历史上下文压缩为记忆向量，预测当前文本。

性质：
- 无 warmup，从训练开始即所有任务均衡
- 全部 2-task 轮换，逻辑对称，无 fallback
- NTP 仅限于"文档首/独立 chunk"，续写仅限于"有前驱 chunk"

**Batch-level n_mem**

同一 batch 内所有非 NTP 样本共享同一个 n_mem 值（由 DataCollator 统一采样）：

| 任务类型 | n_mem | 说明 |
|:---|:---|:---|
| NTP | 0 | 无 MemE 参与，纯语言建模 |
| 重建 | batch-level uniform [1, 128] | batch 内所有非 NTP 样本共享同一值 |
| 续写 | batch-level uniform [1, 128] | 同上 |

Batch-level n_mem 带来两个好处：
1. **零计算浪费**：MemE 只计算 n_mem 个 MEM token 的 hidden states，全部被使用，无丢弃。
2. **Last-token-pool 语义一致**：batch 内每个非 NTP 样本的 Q_N 都占据 MemE 序列的真实 last position。

NTP 独立承担领域知识内化，重建/续写专注于记忆压缩与读取能力训练。

LLM prefix 布局（batch 内 n_mem 统一，无 variable-length 左填充）：
```
NTP sample:                 [target...]                          (无 prefix)
非 NTP sample (n_mem=K):  [<mem>] [h_1, ..., h_K] [</mem>] [target...]
```
...
| **续写** | 当前 chunk 有后续 chunk | **不加**（文本继续） |
| （仅有前驱 chunk 参与） | 当前 chunk 无后续 chunk | **加 EOS**（文档结束） |
...
```
 ...
 目的 │ LLM 内化领域知识 │ 学习压缩→解压能力    │ 学习压缩→续写能力    │
      ...
      ├──────────────────┼──────────────────────┼──────────────────────┤
 n_mem│ 0 (无 latent)    │ batch-level [1, 128] │ batch-level [1, 128] │
      ├──────────────────┼──────────────────────┼──────────────────────┤
 比例 │ 1/3              │ 1/3                  │ 1/3                  │
      └──────────────────┴──────────────────────┴──────────────────────┘
```
...
| 组件 | 学习率 (LR) | 说明 |
|---|---|---|
| **Projector / MEM Embeddings** | `5e-4` | 随机初始化组件，使用高 LR 快速对齐语义空间 |
| **MemE Backbone** | `5e-5` | 较小幅度的微调 |
| **LLM Backbone** | `2e-5` | 最小幅度的微调，保持通用能力 |

> Projector 末层使用近零初始化（std=1e-4），使训练初期 LLM 接收到约为零的 memory prefix，起到隐式 warmup 效果——projector 在不干扰 LLM 的前提下，借助梯度逐步建立 MemE→LLM 映射，无需显式冻结 LLM。
...
# Forward Pass
# 重建任务示例：
split_x + [Q_1, ..., Q_N] → MemE → [v_1, ..., v_N]
    → Projector → [h_1, ..., h_N]
    → LLM 输入: [<mem>, h_1, ..., h_N, </mem>, <AE>] → 输出: split_x

# 续写任务示例（前驱 chunk 作为 MemE context，当前 chunk 作为 LLM target）：
split_{x-1} + [Q_1, ..., Q_N] → MemE → [v_1, ..., v_N]
    → Projector → [h_1, ..., h_N]
    → LLM 输入: [<mem>, h_1, ..., h_N, </mem>] → 输出: split_x
