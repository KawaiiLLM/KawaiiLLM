...
> **MemE 侧不使用 `<mem>` / `</mem>`**: Qwen3-Embedding-4B 采用 causal attention + last-token pooling。MEM tokens 直接拼接在 context 末尾，MEM_n（最后一个 MEM token）占据 last position，自然继承预训练的全局语义聚合能力。插入 `<mem>` / `</mem>` 不仅无益（MemE 从未见过这些 token），还会让 `</mem>` 占据 last position 而其 hidden state 被丢弃，浪费了最有价值的聚合位置。
...
```python
# 构建 MemE 输入: [text, Q_1, ..., Q_n]
# MEM tokens 直接拼接在 context 末尾，无 <mem>/<mem> 包裹。
# Q_n 占据 last position，继承 Qwen3-Embedding 的 last-token-pool 聚合能力。
...
combined = cat([text_embeds, mem_embeds], dim=1)  # [B, L+n, meme_hidden]
...
# 提取最后 n 个位置的 hidden states（即 MEM tokens）
mem_hidden = outputs.last_hidden_state[:, -n_mem:, :]  # [B, n, meme_hidden]
```

> **Causal attention 下的 MEM token 语义**：每个 MEM token 都能看到全部 context token（它们在 context 之后）。通过 36 层 residual attention 的累积，MEM tokens 的 hidden state 主成分是从 context 聚合的语义信息，而非初始随机 embedding。MEM_n（last position）的聚合质量最接近原版 `last_token_pool`。
...
* **n_mem**：NTP → 0; 重建/续写 → 任意非零哨兵值（collator 会覆盖为 batch 级统一值）。
...
* **`n_mem`（batch 级统一采样）:** 一个 batch 内所有非 NTP 样本共享同一个 n_mem 值（uniform [1, num_mem_tokens]），由 collator 统一采样后覆盖。这带来两个好处：
  1. **零计算浪费**：`encode_context` 处理 n_mem 个 MEM token，所有输出都被使用，无丢弃。
  2. **Last-token-pool 语义一致**：每个非 NTP 样本的最后一个 MEM token (Q_n) 都占据 MemE 序列的真实 last position，完整继承 Qwen3-Embedding causal attention 的全局聚合能力。
...
* **Special Token 注册:** 调用 `tokenizer.add_special_tokens({"additional_special_tokens": ["<mem>", "<mempad>", "</mem>", "<AE>"]})` 注册 4 个特殊 token，并 resize MemE 和 LLM 的 embedding 层。`<mem>` / `</mem>` 仅在 LLM 侧作为 prefix 边界标记使用，MemE 侧不使用。
...
> **全参数训练**: `--freeze_meme False`，三组件差异化学习率。Projector LR 最高（5e-4），让随机初始化的 projector 快速学会 MemE→LLM 映射，避免 LLM 学会忽略近零 memory prefix。MemE LR 居中（5e-5），需要学习 MEM token 压缩行为（预训练目标不含此能力）。LLM LR 最低（2e-5），已有强语言能力，微调幅度最小。Flash Attention 2 同时对 MemE 和 LLM 生效。
...
| **n_mem 策略** | 3-phase 阶梯 per-batch | NTP: 0; 重建/续写: collator 统一采样 uniform [1,128]，batch 内所有非 NTP 样本共享同一值 | 零计算浪费；每个样本的 Q_n 都占据 MemE 序列 last position，继承 last-token-pool 全局聚合 |
...
| Per-component LR | 未包含 | `projector_lr=5e-4`, `meme_lr=5e-5`, `llm_lr=2e-5`; `create_optimizer()` 分组实现 | Projector 需最高 LR 快速突破近零输出；MemE 需学习 MEM 压缩新行为；LLM 已有强能力，最小调整 |
| **MemE 侧 `<mem>`/`</mem>`** | 包裹 MEM tokens | **移除** — MEM tokens 直接拼接在 context 末尾 | Qwen3-Embedding causal attention + last-token-pool：MEM_n 需占据 last position 继承聚合能力；`</mem>` 占据 last position 但被丢弃是浪费 |
