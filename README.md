...
支持可选的 **LLM 冷启动冻结策略**：在前 N% 步数内将 LLM 学习率强制设为 0，仅训练 MemE 和 Projector，随后线性回升至设定值。
...
**LLM 冻结参数**：
*   `--llm_freeze_ratio` (默认 0.0): 前 N% 步数完全冻结 LLM (LR=0)，避免 Projector 随机初始化破坏 LLM 权重。
*   `--llm_unfreeze_warmup_ratio` (默认 0.01): 解冻后 N% 步数内，LLM LR 从 0 线性回升至 Scheduler 当前值。
...
split_{x-1} + [<mem>, Q_1, ..., Q_N, </mem>] → MemE → [v_1, ..., v_N]
    → Projector → [h_1, ..., h_N]
