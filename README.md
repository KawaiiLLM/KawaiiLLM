...
通过 NTP 学习领域知识，并学会压缩、读取向量记忆。分为两个任务（参考 PCC）：
...
```
L = λ · L_TC + (1 - λ) · L_TR，其中 λ = 0.5
```

##### 课程学习

两个维度的课程：**n_mem 数量**（先多后少）和**续写任务引入**（warmup 后确定性切换）。

**n_mem 采样：先多后少，线性过渡**

训练初期模型编码能力最弱，给多 latent 让信息瓶颈宽松，容易学会"编码-读取"基本机制；后期逐步减少，迫使压缩。位置 1 在所有阶段都被使用，信息自然向其聚集，天然实现 Persona Vector。

各 n_mem 区间的采样概率随 `progress` **线性插值**（无阶梯跳变）：

```
设 p = training_progress (0→1)

P(high:  64-128) = 0.7 - 0.6p     // 70% → 10%
P(mid:   16-64)  = 0.2 + 0.1p     // 20% → 30%
P(low:    2-16)  = 0.1 + 0.2p     // 10% → 30%
P(single:    1)  = 0.3p           //  0% → 30%
```

| progress | P(64-128) | P(16-64) | P(2-16) | P(1) |
|:---------|:----------|:---------|:--------|:-----|
| 0% | 70% | 20% | 10% | 0% |
| 50% | 40% | 25% | 20% | 15% |
| 100% | 10% | 30% | 30% | 30% |
...
```
              latent 数量多              latent 数量少
           ┌──────────────────────┬──────────────────────┐
  重建任务  │  阶段 1a (最简单)     │  阶段 2b (中等)       │
           │  128 latent + 重建    │  4-16 latent + 重建   │
           │  → 学会基本编码机制   │  → 学会极限压缩        │
           ├──────────────────────┼──────────────────────┤
  续写任务  │  阶段 1b (中等)       │  阶段 2a (最难)        │
           │  64-128 latent + 续写 │  8-32 latent + 续写   │
           │  → 学会提取对续写有用  │  → 从少量信息推断后文  │
           │    的信息              │                       │
           └──────────────────────┴──────────────────────┘

训练进度 →  从左到右（n_mem 线性减少）
           从上到下（warmup 后续写自然引入）
```
